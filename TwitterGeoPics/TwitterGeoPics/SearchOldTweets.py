...
#os.chdir("/content/gdrive/My Drive/projects/infographics_dataset_collection/data/raw/")
GEO = Geocoder()
	...
	if 'media' in status['entities']:
		for media in status['entities'].get('media'):
			if media['type'] == 'photo':
				...
				#with urlopen(photo_url) as in_stream, open(file_name, 'wb') as out_file:
		...
		for item in pager.get_iterator():
			if 'text' in item:
				if not no_retweets or not item.has_key('retweeted_status'):
					process_tweet(item, photo_dir, stalk, no_retweets)
